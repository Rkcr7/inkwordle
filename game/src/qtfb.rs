...
impl QtfbClient {
    ...
    pub fn connect(key: i32, format: u8, width: usize, height: usize, bpp: usize) -> io::Result<Self> {
        ...
        if shm_size < width * height * bpp {
            // `ptr` is already mapped here — unmap it before bailing.
            unsafe {
...
fn send_all(fd: RawFd, buf: &[u8]) -> io::Result<()> {
    // Bound the WouldBlock retry loop: a stalled server must not hang the main
    // loop forever. Past the budget we drop this update rather than block.
            ...
            if e.kind() == io::ErrorKind::WouldBlock {
                if std::time::Instant::now() >= deadline {
                    return Err(io::Error::new(
                        ...
                        "send budget exceeded; dropping update",
                    ));
                }
