...
mod quill_ffi {
    extern "C" {
        ...
        pub fn quill_swap_color(x: i32, y: i32, w: i32, h: i32) -> u64;
...
impl Display {
    pub fn open() -> io::Result<(Self, Surface)> {
        ...
        #[cfg(feature = "takeover")]
        {
            unsafe {
                ...
                // Validate before casting: a negative/-1 error return cast to
                // usize becomes huge, and stride*h (the Surface buffer len) then
                // becomes garbage — Surface would write out of bounds.
                let w = quill_ffi::quill_width();
                let h = quill_ffi::quill_height();
                let stride = quill_ffi::quill_stride();
                if w <= 0 || h <= 0 || stride <= 0 {
                    return Err(io::Error::other("quill returned invalid dimensions"));
                }
                if (stride as i64) < (w as i64) * 4 {
                    return Err(io::Error::other("quill stride too small for width (Rgb32)"));
                }
                let len = (stride as usize)
                    .checked_mul(h as usize)
                    .ok_or_else(|| io::Error::other("quill buffer size overflows usize"))?;
                let ptr = quill_ffi::quill_buffer();
                if ptr.is_null() {
                    return Err(io::Error::other("quill buffer null"));
                }
                let surface =
                    Surface::new(ptr, len, w as usize, h as usize, stride as usize, PixFmt::Rgb32);
    ...
    /// Commit the whole panel in COLOR (Gallery 3). Slow (~1-3s, full flash);
    /// use for a deliberate colored render. qtfb (windowed) has no color path,
    /// so it falls back to a normal full refresh.
    pub fn color_refresh(&self, w: usize, h: usize) {
        match self {
            Display::Qtfb(c) => {
                let _ = c.request_full_refresh();
            }
            ...
            Display::Quill => {
                #[cfg(feature = "takeover")]
                unsafe {
                    quill_ffi::quill_swap_color(0, 0, w as i32, h as i32);
                    quill_ffi::quill_process_events();
                }
            }
        }
