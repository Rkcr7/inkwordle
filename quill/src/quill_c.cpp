...
int quill_init() {
    ...
    // Tear down g_app on every failure return: a second quill_init() would
    // otherwise construct a second QCoreApplication and Qt aborts.
    if (!g_fb) { delete g_app; g_app = nullptr; g_fb = nullptr; return 1; }
    g_aux = g_fb->getAuxFramebuffer();
    if (!g_aux) { delete g_app; g_app = nullptr; g_fb = nullptr; return 2; }
...
// Push a region to glass in COLOR. The aux buffer is RGB32, so colored pixels
// written there render in color on the Gallery 3 panel. Color needs a full
// (flashing) refresh and is slow (~1-3s) — use it for a deliberate "commit".
unsigned long quill_swap_color(int x, int y, int w, int h) {
    if (!g_fb) return 0;
    return g_fb->swapBuffers(QRect(x, y, w, h), EPContentType::Color,
